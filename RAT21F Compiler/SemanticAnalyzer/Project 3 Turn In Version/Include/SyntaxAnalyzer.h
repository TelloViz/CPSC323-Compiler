...
public:
  ...
  SyntaxAnalyzer(std::vector<std::pair<std::string, std::string>> tokenizedSource, std::string& outputString);
	...
	void PrintSymbolTable()
	{
		...
		{
			std::cout << "\n" << iter.second.addr << " " << iter.second.value << " " << iter.second.type;
		}
		std::cout << "\n\n\n";
	}
...
private:
	...
	std::map<std::string, std::string> EquivalenceMap
	{
		...
		{"Qualifier", "integer  |  boolean"},		// these are the words "integer" and "boolean" as in the RAT21F type labels
		{"Body", "{  <Statement List>  }"},
		...
		{"Assign", "<Identifier>  =  <Expression>  ;"},												// Complete
		{"If", "if  (  <Condition>  )  <Statement>  <If'>"},											// Complete
		/*{"Return", "return  <Return'>"},*/
		{"Print", "put  (  <Expression>  )  ;"},													// Complete
		{"Scan", "get  (  <IDs>  )  ;"},															// Complete
		{"While", "while  (  <Condition>  )  <Statement>"},											// Complete
		{"Condition", "<Expression>  <Relop>  <Expression>"},											// Complete
		{"Relop", "==  |  !=  |  >  |  <  |  <=  |  =>"},												// Complete
		...
		{"Integer", "integer"},/*																// Complete
		{"Real", "real"},*/
		{"Expression'", "+  <Term>  <Expression'>  |  -  <Term>  <Expression'>  |  epsilon"},				// + Complete
		{"Term'", "*  <Factor>  <Term'>  |  /  <Factor>  <Term'>  |  epsilon"},							// * Complete
...
#pragma region Symbol Table Handling
	...
	struct Symbol
	{
		Symbol(std::string t, std::string v, bool d, int a) 
			: type{t}, value{v}, isDeclared{d}, addr{a}{}
		...
		int addr{ -1 }; // setting this at -1 by default so if it is misused as an index I'll know at compiler hopefully
	};

	std::map<int, Symbol> symb_table;
	std::map<std::string, int> addr_table;

	bool	insert_symbol(int address, Symbol symbol) 
	{ 
		if (!symbolExists(symbol))
		{
			symbol.isDeclared = true;
			IncrSymbAddr();
			symb_table[address] = symbol;
			addr_table[symbol.value] = address;
	...
	bool		symbolExists(Symbol symbol) { return addr_table.find(symbol) != addr_table.end(); }
	int		get_address(std::string lexeme) 
	{ 
		return addr_table[lexeme]; 
	} // TODO finish implementing symbol table
